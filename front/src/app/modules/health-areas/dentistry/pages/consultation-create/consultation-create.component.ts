...
export default class ConsultationCreateComponent {
  ...
  public onCreateConsultation(formValue: DentistryConsultationForm) {
    ...
    if (request.examen_bucal) {
      request.examen_bucal.id = uuidV4()
    }
    if (request.consulta) request.consulta.id = uuidV4()
    if (request.procedimiento) request.procedimiento.id = uuidV4()

    this.isBlockPage.set(true);
    this._subscription.add(
      this._dentalService.createConsulting(request)
        .subscribe({
          next: (resp) => {
