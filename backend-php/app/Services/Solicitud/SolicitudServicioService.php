...
class SolicitudServicioService
{
    public function updateServicio(array $data): ?Collection
    {
        ...
        foreach ($data['servicios'] as $key => $value) {
            ...
            if ($value['estado'] == 'aprobado') {
                ...
                $servicios = ConvocatoriaServicio::where([['convocatoria_id', $convocatoria->id], ['servicio_id', $servicioSolicitado->servicio_id]])->first();
            ...
            $solicitud->detalle_rechazo = $value['detalle_rechazo'];
    ...
    public function getNumberOfVacancies($id): array
{
    // Obtener el servicio de Comedor
    $serviciosComedor = ConvocatoriaServicio::where([['convocatoria_id', $id], ['servicio_id', 1]])->first();
    $numeroSolicitudesServicioComedor = ServicioSolicitado::whereHas('solicitud', function ($query) use ($id) {
    ...
    // Calcular las vacantes de Comedor, considerando si el servicio existe
    $vacantesComedor = $serviciosComedor ? ($serviciosComedor->cantidad - $numeroSolicitudesServicioComedor) : 0;

    // Obtener el servicio de Internado
    $serviciosInternado = ConvocatoriaServicio::where([['convocatoria_id', $id], ['servicio_id', 2]])->first();
    $numeroSolicitudesServicioInternado = ServicioSolicitado::whereHas('solicitud', function ($query) use ($id) {
        $query->where('convocatoria_id', $id)
              ->where('servicio_id', 2);
    })
    ->where('estado', 'aprobado')
    ...
    // Calcular las vacantes de Internado, considerando si el servicio existe
    $vacantesInternado = $serviciosInternado ? ($serviciosInternado->cantidad - $numeroSolicitudesServicioInternado) : 0;

    return [
        'comedor' => $vacantesComedor,
        'internado' => $vacantesInternado,
    ];
}
