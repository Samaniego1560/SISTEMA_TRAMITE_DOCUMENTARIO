...
class CreateConvocatoriaService
{
    public function create(array $data): Model
    {
        ...
        try {
            $convocatoria = Convocatoria::create([
                ...
                'nombre' => $data['nombre'],
                'user_id' => $user->id,
                'credito_minimo' => $data['credito_minimo'],
    ...
    private function requisitos(array $data, Seccion $secciones): Model
    {
        return $secciones->requisitos()->create([
            ...
            'type_input' => Arr::get($data, 'type_input', 'text'),
            'text_color' => Arr::get($data, 'text_color', 'default'),
            'text_type' => Arr::get($data, 'text_type', 'normal'),
            'text_size' => Arr::get($data, 'text_size', '16'),
            'is_recoverable' => Arr::get($data, 'is_recoverable', false),
            'is_dependent' => Arr::get($data, 'is_dependent', false),
            'field_dependent' => Arr::get($data, 'field_dependent', ''),
            'value_dependent' => Arr::get($data, 'value_dependent', ''),
            'show_dependent' => Arr::get($data, 'show_dependent', false),
